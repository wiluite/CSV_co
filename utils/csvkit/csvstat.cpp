...
namespace csvstat { 
    ...
    void number_class<B>::prepare() {
        ...
        if (!null_number) {
            ...
            std::nth_element(slice.begin(), mm, slice.end(), [](auto &elem, auto &elem2) {
                auto const left = elem.is_null() ? std::numeric_limits<long double>::max() : elem.num();
                auto const right = elem2.is_null() ? std::numeric_limits<long double>::max() : elem2.num();
                return left < right;
            });
    ...
    void text_class<B>::prepare() {
        ...
        for (auto const & e : slice) {
            if (!B::blanks() or !e.is_null()) {
                ...
                null_number++;
            assert(e.is_str() || e.is_null() || e.is_boolean() || e.is_num());
        }
    ...
    void date_class<B>::prepare() {
        ...
        for (auto const & elem : slice) {
            assert(std::get<0>(elem.date()) || elem.is_null());
            if (!elem.is_null()) {
    ...
    void datetime_class<B>::prepare() {
        ...
        date::sys_seconds max_value;
        for (auto const & elem : slice) {
            assert(std::get<0>(elem.datetime()) || elem.is_null());
            if (!elem.is_null()) {

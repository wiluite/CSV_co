...
//TODO: -no-table option
    ...
    template <typename T>
    auto obtain_origins_and_headers(auto const & args) {
        auto rows = 0u;
        std::unordered_set<std::string> header_fields;
        std::vector<std::vector<std::string>> headers;
        for (auto const & elem : args.files) {
            auto reader = T {std::filesystem::path{elem}};
            skip_lines(reader, args);
            auto const header = obtain_header_and_<skip_header>(reader, args);
            check_max_size(reader, args, header_to_strings<csv_co::unquoted>(header), init_row{1});
    ...
    template<typename T>
    auto put_first(auto const & args, auto && table, auto const & group_names) {
        auto reader = T{std::filesystem::path{args.files[0]}};
        skip_lines(reader, args);
        quick_check(reader, args);
        obtain_header_and_<skip_header>(reader, args);
        std::size_t row_idx = 0;
        max_field_size_checker size_checker(reader, args, reader.cols(), init_row{args.no_header ? 1u : 2u});
    ...
    template<typename T>
    void put_rest(auto row_idx, auto const & args, auto && table, auto const& replace_vec, auto const & group_names) {
        auto replace_idx = 0ul;
        auto group_idx = 1ul;
        std::for_each(args.files.begin()+1, args.files.end(),[&](auto const & fn) {
            auto reader = T{std::filesystem::path{fn}};
            skip_lines(reader, args);
            quick_check(reader, args);
            obtain_header_and_<skip_header>(reader, args);
            unsigned const cols = reader.cols();
    ...
    void stack(auto const & args) {
        ...
        auto [rows, cols, headers] = obtain_origins_and_headers<Type>(args);

        std::vector<unsigned> replace_vec;
        auto header = fill_replace_vec(headers, replace_vec, args);
        std::vector<std::vector<std::string>> t (rows, std::vector<std::string>(cols +(args.groups.empty() && !args.filenames ? 0 : 1)));
        put_rest<Type>(put_first<Type>(args, t, group_names), args, t, replace_vec, group_names);
...
int main(int argc, char * argv[]) {
    ...
    try {
        if (!args.skip_init_space) 
            ...
            stack<skipinitspace_reader_type>(args);
